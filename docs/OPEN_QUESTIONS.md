...
| 11 | **Video-quote unit normalisation.** Per-commodity unit inference now built (`analytics/unit_inference.py`): Onion/Capsicum/Garlic/Mango/Ginger → ₹/kg; interim study deviation dropped **370% → 41.5%**. **Remaining:** Potato/Tomato/Pomegranate show a ~15–19× ratio (not a clean 100× quintal) — likely quoted **per-crate/pallı**; the code honestly marks them `unknown` and excludes them rather than guess. These need a per-crate weight (a `unit_convention` row) or invoice cross-check to resolve. | M5 interim finding | before M5 real verdict | PARTIALLY RESOLVED — kg/quintal inferred; per-crate commodities still open. |
